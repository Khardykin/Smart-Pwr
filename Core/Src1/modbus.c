...
const TVAR reg_x03[] =
{
		/* 0x1000 */	{ (uint16_t*)&mbDevStatus,					FALSE },	/* Состояние 							*/
		...
		/* 0x1022 */	{ (uint16_t*)&(HIWORD(stMain.Serial)),		TRUE },		/* Заводской номер прибора Hi			*/
		/* 0x1023 */	{ (uint16_t*)&(LOWORD(stMain.Serial)),		TRUE },		/* Заводской номер прибора Lo			*/
...
const tINREG reg_x04[]=
{
		...
		/* 0x0003 */	(uint16_t*)&(HIWORD(stMain.Serial)),				/* Заводской номер HI							*/
		/* 0x0004 */	(uint16_t*)&(LOWORD(stMain.Serial)),				/* Заводской номер LO							*/
		...
		/* 0x0020 */	(uint16_t*)&Reserv,					/* Значение АЦП после температурной коррекции			*/
